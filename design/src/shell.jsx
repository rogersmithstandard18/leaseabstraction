// Folio — app shell: sidebar, topbar, role switcher.
const { useState: shS, useEffect: shE, useRef: shR, useMemo: shM } = React;
...
function Sidebar({ tweaks, role, page, setPage, openRoleSwitcher }) {
  ...
  const counts = shM(() => {
    const open = window.INCIDENTS.filter(i => i.status !== "Abstracted").length;
    const crits = window.INCIDENTS.filter(i => i.severity === "Critical" && i.status !== "Abstracted").length;
    ...
    <aside className="sidebar">
      <div className="sb-brand">
        ...
        <div>
          ...
          <div className="sb-brand-sub">Lease Intelligence</div>
        </div>
      </div>
      ...
      <div style={{ padding: "0 8px", display: "flex", flexDirection: "column", gap: 4 }}>
        <MiniStat label="Critical in queue" value={counts.crits} tone="bad" />
        <MiniStat label="Open total" value={counts.queue} tone="muted" />
        <MiniStat label="Accuracy · 7d" value="99.2%" tone="good" />
      </div>

      <div className="sb-foot">
        <div className="sb-user" onClick={openRoleSwitcher} title="Switch role / altitude">
...
function Topbar({ role, page, focusedIncident }) {
  ...
  return (
    <div className="topbar">
      ...
      <div className="topbar-pill" title="Pipeline status">
        <span className="dot" />
        <span>Pipeline nominal · 4.1 leases/hr</span>
      </div>
      <div className="topbar-search" onClick={() => alert("Cmd-K (mock)")}>
        <Icon name="search" />
        <span>Search leases, tenants, clauses, fields…</span>
        <kbd>⌘K</kbd>
      </div>
      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-head">
          ...
          <div style={{ fontSize: 12.5, color: "var(--text-3)", marginTop: 4 }}>
            The same lease data, rendered at the altitude this person operates at.
          </div>
        </div>
