// Folio — small reusable display primitives.
...
function KindGlyph({ kind, size = 22 }) {
  ...
  return (
    <span style={{
      ...
      fontFamily: "var(--font-mono)", fontSize: size * 0.6, fontWeight: 600, lineHeight: 1,
    }}>{m.glyph}</span>
  );
}
...
function ConfidenceRing({ value, size = 44 }) {
  const tone = value >= 95 ? "var(--good)" : value >= 85 ? "var(--info)" : value >= 70 ? "var(--warn)" : "var(--bad)";
  const bg = value >= 95 ? "var(--good-bg)" : value >= 85 ? "var(--info-bg)" : value >= 70 ? "var(--warn-bg)" : "var(--bad-bg)";
  const s = size, r = (s / 2) - 4, c = 2 * Math.PI * r, off = c * (1 - value / 100);
  return (
    <div className="ring" style={{ width: s, height: s, background: bg, color: tone, position: "relative" }}>
      <svg width={s} height={s} style={{ position: "absolute", inset: 0, transform: "rotate(-90deg)" }}>
        <circle cx={s/2} cy={s/2} r={r} stroke="currentColor" strokeOpacity="0.18" strokeWidth="2.5" fill="none" />
        <circle cx={s/2} cy={s/2} r={r} stroke="currentColor" strokeWidth="2.5" fill="none"
...
function PropertyThumb({ kind, w = 56, h = 40 }) {
  const m = window.KIND_META[kind] || { color: "#6B7280" };
  return (
    <div className="plot" style={{ width: w, height: h, borderColor: m.color + "30" }}>
      <span style={{ color: m.color + "AA" }}>{m.glyph}</span>
    </div>
  );
}
...
Object.assign(window, { SevChip, StatusChip, KindGlyph, Sparkline, MiniBar, Dollars, TimeAgo, Section, ConfidenceRing, PropertyThumb });
